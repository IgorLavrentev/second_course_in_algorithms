...
class SimpleTree:
    ...
    def __number_of_nodes(
        ...
        for j in list_Children:
            self.count_Nodes += 1
            if j.Children != []:
                self.__number_of_nodes(count_Nodes, j.Children, count)

    def number_of_nodes(self, r) -> int:
        ...
        for i in r.Children:
            ...
            if i.Children != []:
                count: int = 0
                list_Children: list = i.Children
                self.__number_of_nodes(self.count_Nodes, list_Children, count)
    ...
    def __EvenTrees(
        self, list_Children: list, count: int, even_trees_list: list
    ) -> None:
        if count == len(list_Children) and list_Children == []:
            return
        count += 1
