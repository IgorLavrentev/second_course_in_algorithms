...
class BST:
    ...
    def __WideAllNodes_level(self, node, level, list_BSTNode) -> None:
        if node is None:
            return
        if level == 0:
            list_BSTNode.append(node)
        if level > 0:
            self.__WideAllNodes_level(node.LeftChild, level - 1, list_BSTNode)
            self.__WideAllNodes_level(node.RightChild, level - 1, list_BSTNode)

    def WideAllNodes(self) -> list:
        if self.Root is None:
            return 0
        node: BSTNode = self.Root
        level: int = self.__height(self.Root)
        list_BSTNode: list = []
