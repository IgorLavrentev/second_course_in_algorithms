...
class SimpleGraph:
    ...
    def searching_nodes_class_interface(self):
        ...
        for i in range(len(self.__vertex)):  # цикл по всем элементам графа
            ...
            summ = 0
            for el in range(len(self.__vertex)):
                if not self.IsEdge(self.__vertex[i].Value, self.__vertex[el].Value):
                    summ += 1
            if summ == len(self.__vertex):
                res_list.append(self.__vertex[i])
    ...
    def search_nodes_matrix_contiguity(self):
        list_links = []
        res_list = []
        # формирование связей очередного элемента
        for j in range(len(self.__m_adjacency)):
            for k in range(len(self.__m_adjacency)):
                if self.__m_adjacency[k][j] == 1:
                    list_links.append(self.__vertex[k])
        # проверка связей между элементами
            flag = True
            for el in range(len(list_links)):
                for el_1 in range(len(list_links)):
                    if self.__m_adjacency[list_links[el_1].Value][list_links[el].Value] == 1:
                        flag = False
                        break
            if flag == True:
                res_list.append(self.__vertex[j])
            list_links = []
        return res_list

    def search_nodes_matrix_with_BFS(self):
        list_links = []
        res_list = []
        ...
        for i in range(len(self.__vertex)):  # цикл по всем элементам графа
            ...
            for j in range(len(self.__vertex)):
                if i == j:
                    continue
                if self.IsEdge(self.BreadthFirstSearch(0, i)[-1].Value, self.BreadthFirstSearch(0, j)[-1].Value):
                    list_links.append(self.__vertex[j])
            ...
            for k in range(len(list_links)):
                ...
                for k_2 in range(len(self.__vertex)):
                    if (
                        self.IsEdge(list_links[k].Value, self.BreadthFirstSearch(0, k_2)[-1].Value)
                        and self.__vertex[k_2] in list_links
                        and k_2 != list_links[k].Value
                    ):
                        flag = False
